...
await writeFile(installer, 'closed-loop-fixture')
...
try {
  ...
  assert.equal(trashEntries.some(name => name.startsWith('安装包')), true)
  assert.deepEqual(errors, [])
  console.log(JSON.stringify({ closedLoop: 'passed', tool: 'move_to_trash', fileMoved: true, rendererErrors: errors.length }))
} finally {
