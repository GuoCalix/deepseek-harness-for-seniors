...
function App() {
  const [locale, setLocale] = useState<Locale>(() => (localStorage.getItem('ai-old-locale') as Locale | null) ?? 'zh')
  ...
  async function startTask() {
    ...
    try {
      const result = await api.createTask(prompt.trim())
  ...
  function openTask(item: Task) {
    setSelectedId(item.id); setCandidates(item.status === 'CLARIFYING' || item.status === 'REVISION' ? ((item as Task & { candidates?: Candidate[] }).candidates ?? candidates) : []); setPreview(item.status === 'READY_TO_RUN' ? { target: item.turns.at(-1)?.content ?? item.title, roots: item.accessScope?.roots ?? ['桌面', '下载'], output: 'workspace/output/任务说明.md', network: '仅在你配置 DeepSeek API key 时联网' } : null); setView('home'); setFeedback(''); setFeedbackComment('')
  }
  async function submitClarification() {
    ...
    if (!content) return
    setBusy(true)
    try { const result = await api.clarify(task.id, content); updateTask(result.task); setPreview(result.preview); setSelectedCandidate(null); setCustomClarification('') }
  ...
  async function approveAndRun() {
    ...
    try {
      await api.approveAccess(task.id, preview?.roots ?? [], false)
      const result = await api.run(task.id); updateTask(result)
  ...
  async function sendFeedback() {
    ...
    setBusy(true)
    try { const result = await api.feedback(task.id, feedback, feedbackComment); updateTask(result); setFeedback(''); setFeedbackComment('') }
  ...
  return <div className={`app-shell ${largeText ? 'large-text' : ''} ${highContrast ? 'high-contrast' : ''}`}>
    ...
    <main className="main-column">
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          <button className="account-chip" onClick={() => setLoginOpen(true)}><span className="avatar">林</span><span className="account-chip-copy"><strong>{t.connected}</strong><small>deepseek-chat</small></span><ChevronRight size={16} /></button>
        </div>
      </header>
    ...
    {loginOpen && <LoginModal t={t} onClose={() => setLoginOpen(false)} />}
  </div>
}
...
function ProgressCard({ t, task }: { t: Copy; task: Task }) { return <div className="timeline-card progress-card"><div className="progress-heading"><div className="progress-orb"><LoaderCircle className="spin" size={28} /></div><div><h2>{t.runningTitle}</h2><p>{t.runningHint}</p></div></div><div className="progress-track"><span style={{ width: `${Math.max(22, task.completedSteps / task.totalSteps * 100)}%` }} /></div><div className="progress-stats"><span><strong>{task.foundFiles}</strong>{t.found}</span><span><strong>{task.completedSteps}/{task.totalSteps}</strong>{t.steps}</span><span><strong>{task.elapsedSeconds}s</strong>{t.elapsed}</span></div><div className="progress-actions"><button className="secondary-button"><Pause size={17} />{t.pause}</button><button className="ghost-button">{t.details}</button></div></div> }
function ResultCard({ t, task, feedback, setFeedback, feedbackComment, setFeedbackComment, onFeedback, busy }: { t: Copy; task: Task; feedback: string; setFeedback: (value: string) => void; feedbackComment: string; setFeedbackComment: (value: string) => void; onFeedback: () => void; busy: boolean }) { return <div className="timeline-card result-card"><div className="result-banner"><span className="success-icon"><Check size={24} /></span><div><h2>{t.completedTitle}</h2><p>{t.completedHint}</p></div></div><div className="workspace-path"><FolderOpen size={18} /><div><small>{t.folderPath}</small><strong>{task.workspacePath ?? 'workspace/output'}</strong></div><CopyButton value={task.workspacePath ?? ''} t={t} /></div><div className="artifact-heading"><h3>{t.artifacts}</h3><span>{task.artifacts.length}</span></div><div className="artifact-list">{task.artifacts.map(artifact => <div className="artifact-row" key={artifact.id}><span className="artifact-icon"><FileText size={19} /></span><div><strong>{artifact.name}</strong><small>{bytes(artifact.size)} · {dateLabel(artifact.generatedAt, 'zh')}</small></div><button className="icon-button subtle" aria-label={t.preview}><ExternalLink size={17} /></button></div>)}</div><div className="input-summary"><div className="summary-title"><Search size={17} />{t.inputSummary}</div><p>{task.foundFiles} {t.found} · {t.localFirst}</p></div><div className="feedback-box"><div className="feedback-title"><h3>{t.feedbackTitle}</h3><span>{t.feedbackHint}</span></div><div className="feedback-options"><button className={`feedback-option good ${feedback === t.good ? 'selected' : ''}`} onClick={() => setFeedback(t.good)}><Check size={17} />{t.good}</button>{t.feedback.map(item => <button className={`feedback-option ${feedback === item ? 'selected' : ''}`} key={item} onClick={() => setFeedback(item)}>{item}</button>)}</div>{feedback && feedback !== t.good && <textarea className="feedback-input" rows={2} value={feedbackComment} onChange={event => setFeedbackComment(event.target.value)} placeholder={t.commentHint} />}{feedback && <button className="primary-button feedback-submit" onClick={onFeedback} disabled={busy}>{busy ? <LoaderCircle className="spin" size={18} /> : <Check size={18} />}{t.submitFeedback}</button>}</div></div> }
...
function AccountView({ t, onLogin }: { t: Copy; onLogin: () => void }) { return <div className="page-view"><PageTitle icon={<WalletCards size={23} />} title={t.account} subtitle={t.connected} /><div className="balance-grid"><div className="balance-card dark"><div className="balance-card-top"><span>{t.balance}</span><WalletCards size={21} /></div><strong>¥ 28.60</strong><small>DeepSeek API credit · demo balance</small><button className="light-button" onClick={onLogin}>{t.topUp}<ChevronRight size={16} /></button></div><div className="balance-card"><div className="balance-card-top"><span>{t.monthly}</span><Clock3 size={21} /></div><strong>42.8K</strong><small>tokens · September 2026</small><div className="usage-bar"><span style={{ width: '34%' }} /></div></div></div><section className="account-section"><div className="section-heading"><div><h2>{t.login}</h2><span>{t.loginHint}</span></div><button className="primary-button" onClick={onLogin}><LockKeyhole size={18} />{t.signIn}</button></div><div className="account-security"><ShieldCheck size={21} /><div><strong>{t.localFirst}</strong><p>{t.localHint}</p></div></div></section><section className="account-section"><div className="section-heading"><div><h2>{t.topUp}</h2><span>{t.orderHint}</span></div></div><div className="amount-options"><button>¥ 20</button><button className="selected">¥ 50</button><button>¥ 100</button><button>{t.custom}</button></div></section></div> }
function SettingsView({ t, locale, setLocale, largeText, setLargeText, highContrast, setHighContrast }: { t: Copy; locale: Locale; setLocale: (locale: Locale) => void; largeText: boolean; setLargeText: (value: boolean) => void; highContrast: boolean; setHighContrast: (value: boolean) => void }) { return <div className="page-view"><PageTitle icon={<Settings size={23} />} title={t.settings} subtitle={t.localFirst} /><div className="settings-layout"><section className="settings-section"><h2>{t.language}</h2><div className="segmented"><button className={locale === 'zh' ? 'active' : ''} onClick={() => setLocale('zh')}>{t.chinese}</button><button className={locale === 'en' ? 'active' : ''} onClick={() => setLocale('en')}>{t.english}</button></div></section><section className="settings-section"><h2>{t.scale}</h2><SettingToggle label={t.large} detail="18px → 20px" checked={largeText} onChange={setLargeText} /><SettingToggle label={t.contrast} detail="WCAG AA" checked={highContrast} onChange={setHighContrast} /></section><section className="settings-section"><h2>{t.privacy}</h2><div className="setting-info"><span className="setting-icon"><FolderOpen size={19} /></span><div><strong>{t.workspace}</strong><p>~/Desktop/AI for the old/</p></div><button className="icon-button subtle"><ChevronRight size={17} /></button></div><div className="setting-info"><span className="setting-icon green"><ShieldCheck size={19} /></span><div><strong>{t.localFirst}</strong><p>{t.localHint}</p></div></div></section><section className="settings-section"><h2>{t.tools}</h2><p className="section-description">{t.toolsHint}</p><div className="tool-chips">{['list_files', 'read_metadata', 'copy_files', 'write_text', 'create_spreadsheet', 'open_result'].map(tool => <span key={tool}><Check size={14} />{tool}</span>)}</div></section></div></div> }
...
function LoginModal({ t, onClose }: { t: Copy; onClose: () => void }) { const [qr, setQr] = useState(''); const [opened, setOpened] = useState(false); useEffect(() => { QRCode.toDataURL('https://platform.deepseek.com/sign_in', { width: 210, margin: 1, color: { dark: '#1d2a26', light: '#ffffff' } }).then(setQr).catch(() => undefined) }, []); return <Modal title={t.login} onClose={onClose}><div className="login-modal"><div className="qr-frame">{qr ? <img src={qr} alt="DeepSeek sign-in QR code" /> : <LoaderCircle className="spin" />}</div><h3>{t.loginHint}</h3><p>{t.localHint}</p><button className="primary-button full-button" onClick={() => { window.open('https://platform.deepseek.com/sign_in', '_blank', 'noopener,noreferrer'); setOpened(true) }}><ExternalLink size={18} />{t.openLogin}</button>{opened && <button className="secondary-button full-button" onClick={onClose}><Check size={18} />{t.loggedIn}</button>}</div></Modal> }
