...
function App() {
  ...
  async function approveAndRun() {
    ...
    try {
      await api.approveAccess(task.id, preview?.roots ?? [], true)
...
function AccountView({ t, onLogin }: { t: Copy; onLogin: () => void }) {
  const [status, setStatus] = useState<{ apiConfigured: boolean; webLoginTransfer: boolean } | null>(null)
  useEffect(() => { api.accountStatus().then(setStatus).catch(() => setStatus(null)) }, [])
  return <div className="page-view"><PageTitle icon={<WalletCards size={23} />} title={t.account} subtitle={status?.apiConfigured ? t.connected : t.webLoginNotApi} /><div className="balance-grid"><div className="balance-card dark"><div className="balance-card-top"><span>{t.balance}</span><WalletCards size={21} /></div><strong>--</strong><small>{t.balanceUnavailable}</small><button className="light-button" onClick={onLogin}>{t.openLogin}<ExternalLink size={16} /></button></div><div className="balance-card"><div className="balance-card-top"><span>{t.monthly}</span><Clock3 size={21} /></div><strong>API</strong><small>{status?.apiConfigured ? t.connected : t.apiKeyRequired}</small><div className="usage-bar"><span style={{ width: status?.apiConfigured ? '100%' : '0%' }} /></div></div></div><section className="account-section"><div className="section-heading"><div><h2>{t.login}</h2><span>{t.webLoginNotApi}</span></div><button className="primary-button" onClick={onLogin}><LockKeyhole size={18} />{t.signIn}</button></div><div className="account-security"><ShieldCheck size={21} /><div><strong>{t.localFirst}</strong><p>{t.localHint}</p></div></div></section><section className="account-section"><div className="section-heading"><div><h2>{t.topUp}</h2><span>{t.billingUnavailable}</span></div><button className="secondary-button" onClick={onLogin}><ExternalLink size={17} />{t.openLogin}</button></div><div className="account-security"><WalletCards size={21} /><div><strong>{t.orderHint}</strong><p>{t.billingUnavailable}</p></div></div></section></div>
}
